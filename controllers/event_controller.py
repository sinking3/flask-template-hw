...
from models.event_list import events_list, add_new_event, remove_event
...
def index():
    return render_template('index.jinja', title='Event List', events=events_list)
...
@event_blueprint.route('/events/delete/<int:index>', methods=["POST"])
def delete(index):
    remove_event(index)
    return redirect("http://127.0.0.1:4998/events")